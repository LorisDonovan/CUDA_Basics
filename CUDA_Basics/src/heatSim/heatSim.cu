...
__global__ void CopyConstKernel(float* dest, const float* src)
{
	int32_t x = threadIdx.x + blockIdx.x * blockDim.x;
	int32_t y = threadIdx.y + blockIdx.y * blockDim.y;
	int32_t offset = x + y * blockDim.x * gridDim.x;

	if (src[offset] != 0) // to preserve previous values
...
void AnimGPU(DataBlock* d, int ticks)
{
	...
	for (int i = 0; i < 90; i++)
	{
		CopyConstKernel<<<blocks, threads>>>(d->d_InSrc, d->d_ConstSrc);
		BlendKernel<<<blocks, threads>>>(d->d_OutSrc, d->d_InSrc);
